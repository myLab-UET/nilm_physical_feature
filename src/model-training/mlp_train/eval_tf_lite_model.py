...
# filepath: /home/mrcong/Code/mylab-nilm-files/nilm-physical-features/src/model-training/mlp_train/eval_tf_lite_model.py
...
tflite_model_path = f"{model_path}/{model_name}"
...
def evaluate_test_dataset(interpreter, test_loader):
    y_true_list = []
    y_pred_list = []
...
# Starting Main():
print(f"[+] Evaluation with model path: {tflite_model_path}")
...
all_y_true, all_y_pred = evaluate_test_dataset(interpreter=interpreter, test_loader=val_loader)
...
logging.info(f"[+] TFLite results: {model_name} with {data_type} data")
